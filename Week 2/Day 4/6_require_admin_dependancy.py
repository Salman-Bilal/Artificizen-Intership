"""
Question: 6: Add a role field to your user model. Write a require_admin dependency that raises 403 if the current user is not an admin, and apply it to a DELETE /users/{id} route.
...
# ✨ TASK STEP 2: Write the require_admin dependency
...
def register_user(user_data: UserRegister, db: Session = Depends(get_db)):
    ...
    new_user = DBUser(
        ...
        role=user_data.role  # Save the registration role!
    )
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
